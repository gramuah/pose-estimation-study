...
class PoseLabelGenerator(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        top[1].reshape(*pose_weights.shape)
        top[1].data[...] = pose_weights
...
class ContinuousPoseLabelGenerator(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        azimuth_labels, weights_vector = self._get_pose_regression_labels(fg_azimuths, class_labels, self._num_classes)
        # Forward data
        top[0].reshape(*azimuth_labels.shape)
        top[0].data[...] = azimuth_labels
        top[1].reshape(*weights_vector.shape)
