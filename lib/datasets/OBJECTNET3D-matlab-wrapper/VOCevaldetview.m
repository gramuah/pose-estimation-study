function VOCevaldetview(comp_id, set_eval, detections_folder, result_dir)
...
% ObjectNet3D path
root = '../../../data/ObjectNet3D/';

if exist(result_dir, 'dir') == 0
    display(['Results directory does not exist:', result_dir]);
    return;
end
...
% load test set
gtids = textread(sprintf('%s/Image_sets/%s.txt', root, set_eval), '%s');
M = numel(gtids);
...
parfor k = 1:num_cls
    ...
    % load detections
    filename = sprintf('%s/%s_%s_%s.txt', detections_folder, comp_id, cls, set_eval);
...
fid = fopen(sprintf('%s/sumary_%s_%s.txt', result_dir, comp_id, set_eval), 'w');
for i = 1:num_cls
...
% save to matfile
matfile = sprintf('%s/sumary_%s_%s.mat', result_dir, comp_id, set_eval);
