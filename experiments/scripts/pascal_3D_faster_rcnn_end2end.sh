...
LOG="experiments/logs/TUNE_faster_rcnn_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_net.py \
	--gpu ${GPU_ID} \
	--solver models/VGG16/faster_rcnn_end2end/solver_3Dplus_4.prototxt \
	--weights ${NET_INIT} \
	--imdb ${DATASET_TRAIN} \
	--iters ${ITERS} \
...
time ./tools/test_net.py --gpu ${GPU_ID} \
  --def models/VGG16/faster_rcnn_end2end/test_3Dplus_4.prototxt \
  --net ${NET_FINAL} \
  --imdb ${DATASET_TEST} \
  --cfg experiments/cfgs/faster_rcnn_end2end.yml \
  ${EXTRA_ARGS}
