...
LOG="experiments/logs/3DPLUS_NETWORK-SPECIFIC_(train-val).txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_net.py \
	--gpu ${GPU_ID} \
	--solver models/VGG16/faster_rcnn_end2end/solver_3Dplus_network-specific.prototxt \
	--weights ${NET_INIT} \
	--imdb ${DATASET_TRAIN} \
	--iters ${ITERS} \
	--network_specific \
